...
def get_by_url(url = None): 
    ...
    return json.dumps(vm.to_video(data), indent=4), 200, {'ContentType':'application/json'}
...
def update_video(id = None): 
    ...
    data, has_error = try_action(lambda: vm.get_video_by_id(id))
    if has_error: return f"{{\"message\": \"{data}\"}}", has_error, {'ContentType':'application/json'}
    if data.get('total') == 0: return f"{{\"message\":\"Unknown ID\"}}", 400, {'ContentType':'application/json'}
...
def new_videos(): 
    ...
    return f"{{\"results\": {response}}}", 200, {'ContentType':'application/json'}
...
def add_video(user_data): 
    ...
    if has_error: return f"{{\"message\": \"{_data}\"}}", has_error, {'ContentType':'application/json'}
